...
"""Hi3DGen を gfx1151 / Windows / ROCm で動かすための**起動側のシム**。

**`runners/trellis/shims.py` と同じ内容を意図して複製している。** ランナーは互いを
import しない（それぞれ独立リポジトリへ出すため）。片方を直したらもう片方も直すこと。
...
Hi3DGen は FlexiCubes と text→3D を持たないので `kaolin` と `open3d` は実際には要らないが、
**同じ内容を保つほうが間違いが起きない**ので置いたままにしてある（触らなければ何もしない）。
