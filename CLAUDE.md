# CLAUDE.md — working on hi3dgen-strix-halo

**Hi3DGen / Stable3DGen image-to-mesh on AMD Strix Halo (gfx1151), Windows,
ROCm.** It goes image -> normal map -> mesh.
...
1. **Upstream code is never modified.** It is cloned and run as it is, because a
   ...
   hardware needs is injected at launch time**, from `runners/hi3dgen/shims.py`
...
9. **`shims.py` is deliberately a copy of the TRELLIS runner's**, because
   Hi3DGen's sparse modules are TRELLIS code. The two never import each other,
   since each ships as its own repository: **fix one and fix the other.**
