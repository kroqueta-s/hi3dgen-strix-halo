...
from runners.hi3dgen import config  # noqa: E402  (config reads dotenv only, no torch)
...
def main() -> int:
    ...
    from runners.hi3dgen import gfxlight, pipeline
    ...
    try:
        ...
        stage_walls = {
            "rembg": warm.preprocess_sec,
            "normal": warm.normal_sec,
            "cond": warm.cond_sec,
        ...
        pipe, normal_predictor = pipeline.load_pipeline(say)
        ...
        foreground = profiled("rembg", lambda: pipe.preprocess_image(image, resolution=1024))
        normal_image = profiled(
            "normal",
            lambda: normal_predictor(
                foreground,
        ...
        with torch.no_grad():
            cond = profiled("cond", lambda: pipe.get_cond([normal_image]))
