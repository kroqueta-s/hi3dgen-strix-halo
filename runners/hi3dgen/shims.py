...
"""**Launch-time shims** that make Hi3DGen run on gfx1151 / Windows / ROCm.

**This file is deliberately a copy of the TRELLIS runner's `shims.py`**
(`trellis-strix-halo`), because Hi3DGen's sparse modules are TRELLIS code.
The two runners never import each other, since each ships as its own
repository. **Fix one and fix the other.**

**Upstream code is never modified** (a re-clone or an update would undo it).
`install()` is called **before** importing the upstream package and puts the
replacements into `sys.modules`.

Four things are replaced. **Each is on the required path but unobtainable on
Windows + ROCm**, and the surface of API actually used is small. Hi3DGen has
neither FlexiCubes nor text-to-3D, so `kaolin` and `open3d` are not truly
needed here, but **keeping both files identical prevents mistakes**, so they
stay (they do nothing unless touched).
