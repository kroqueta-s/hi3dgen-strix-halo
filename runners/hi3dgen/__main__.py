...
"""The Hi3DGen runner (an implementation of the runner contract).
...
# sparse-conv shim's skinny GEMM ~8x faster than rocBLAS (measured 2026-09-02,
...
def m_capabilities(params: dict[str, Any], progress: Any) -> dict[str, Any]:
    """Report capabilities. **Answers immediately, without loading the model.**"""
    return {
        "name": NAME,
        "version": VERSION,
        ...
        "capabilities": {
            ...
            # Upstream is built around normals only; there is no texture stage.
            "texture": False,
        },
        "params": {
            "ss_steps": {"type": "int", "default": 50, "min": 1, "max": 100},
            "slat_steps": {"type": "int", "default": 6, "min": 1, "max": 100},
            "ss_guidance": {"type": "float", "default": 3.0, "min": 0.0, "max": 20.0},
        ...
        "notes": (
            ...
            "launch-time shims. The mesh comes back Z-up at normalized scale. No texture."
        ),
    }


def m_load(params: dict[str, Any], progress: Any) -> dict[str, Any]:
    """Load the weights (3D pipeline, BiRefNet, StableNormal)."""
    from . import pipeline

    progress("load", "loading Hi3DGen weights (including normal estimation and background removal)")
...
def m_image_to_mesh(params: dict[str, Any], progress: Any) -> dict[str, Any]:
    ...
    **Background removal and normal estimation are this runner's job.**
    **Scaling to real-world size is not.** Millimetres are downstream work
    (meshforge's forge).
    """
    ...
    progress("shape", "background removal, normal estimation, then 3D generation (several minutes)")
    ...
    return {
        ...
        "metrics": {
            ...
            "preprocess_sec": round(result.preprocess_sec, 2),
            "normal_sec": round(result.normal_sec, 2),
            ...
            # **Which BLAS backend served the GEMMs.** They differ by up to 8x
